...
class ChunkingPipeline:
    def __init__(self):
        ...
        # Product categories and synonyms for drones
        self.categories = {
            'mini-toy-drone': ['mini', 'toy', 'basic', 'beginner', 'small'],
            'professional-drone': ['professional', 'pro', 'advanced', 'commercial', 'enterprise'],
            'camera-drone': ['camera', '4k', 'photo', 'video', 'aerial'],
            'racing-drone': ['racing', 'fpv', 'speed', 'race', 'competition'],
            'agriculture-drone': ['agriculture', 'farming', 'spray', 'crop', 'field']
        }
...
def main():
    ...
    for _, row in df.iterrows():
        if isinstance(row['title'], str) and isinstance(row['category'], str):
            combined_text = f"{row['title']} - {row['category']}"
    ...
    for strategy, chunks in chunk_results.items():
        ...
        test_queries = [
            "4k camera drone",
            "professional drone with GPS",
            "mini toy drone for beginners"
        ]
